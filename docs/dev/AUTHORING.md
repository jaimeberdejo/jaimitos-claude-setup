...
| An override records WHY it differs from the recommendation | **Human-dependent** — `classify-work.sh` warns on a reasonless override but still exits 0, and nothing checks the *reason* text reached `docs/SPEC.md`; at the `/phase` gate `plan-review-route.sh` does render a reasonless override structurally as `reason: MISSING` |
| The persisted `docs/SPEC.md` `tier:` is a valid value; an invalid or absent one does not buy reduced review | **Deterministic** — `plan-review-route.sh` validates the value and fails safe to STANDARD + full PLAN_CHECK. Deterministic over the *declared* tier, not over the implementation |
| A clear low-risk STANDARD skips the full PLAN_CHECK; risky STANDARD / DEEP / supervised / high-stakes / hard-stale / blocking-clarification run it | **Deterministic routing + model-dependent review** — `plan-review-route.sh` picks the route from the declared tier + the plan's **declared** paths (a path matcher via `_high-stakes.sh`) + freshness; whether that route is *appropriate* for the actual work stays model/human-dependent (no code exists at plan time) |
